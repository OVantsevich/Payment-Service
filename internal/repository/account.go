...
func (r *Account) GetUserAccount(ctx context.Context, id string) (*model.Account, error) {
	account := model.Account{}
	err := r.QueryRow(ctx, `select id, amount from accounts where user=$1 and deleted=false`, id).Scan(
...
func (r *Account) GetUserAccountForUpdate(ctx context.Context, id string) (*model.Account, error) {
	account := model.Account{}
	err := r.QueryRow(ctx, `select id, amount from accounts where user=$1 and deleted=false for update`, id).Scan(
		&account.ID, &account.Amount)
	if err != nil {
...
func (r *Account) UpdateAmount(ctx context.Context, id string, amount float64) error {
	_, err := r.Exec(ctx, `update accounts set amount=amount+$1 where user=$2 and deleted=false`, amount, id)
	if err != nil {
